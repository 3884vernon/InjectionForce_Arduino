...
def improved_exp_fit_func(V, debug=False, mask=None, min_lifetime=10, max_lifetime=250):
    """
    ...
    mask : numpy.ndarray, optional
        Boolean mask to limit fitting to specific regions
    min_lifetime : float
...
def visualize_enhanced_results(base_image, lifetime_map, certainty_map, masks):
    """
...
def analyze_condition_folder(folder_path, manual_circles, results_list, debug=True):
    ...
    lifetime_map, certainty_map = improved_exp_fit_func(stack, debug=True)

    # Visualize results
    if debug:
        visualize_enhanced_results(base_image, lifetime_map, certainty_map, masks)
    ...
    for i, mask in enumerate(masks):
        ...
        masked_lifetime, _ = improved_exp_fit_func(stack, debug=True, mask=mask)
...
def test_exp_fit():
    ...
    # Test the function
    test_lifetime_map, test_certainty_map = improved_exp_fit_func(test_vol, debug=True)
