...
def exp_fit_func(V, debug=False, mask=None):
    """
    ...
    for i in range(H):
        for j in range(W):
            ...
            try:
                betac, residuals, _, _ = np.linalg.lstsq(XX, lny, rcond=None)
                a = betac[1]  # slope

                # Skip if slope is positive or too flat (poor exponential decay)
                if a >= 0:  # Changed from a <= 0 to a >= 0 since decay should have negative slope
                    skip_flat += 1
                    continue
                ...
                if debug and mask is not None and mask[i, j]:
                    debug_values.append((i, j, a, lifetime))
    ...
    if debug:
        ...
        print(f"Skipped due to negative values: {skip_negative} ({skip_negative / max(1, total_pixels) * 100:.1f}%)")
        ...
        if mask is not None and debug_values:
            ...
            if debug_values:
                ...
                for idx in random.sample(range(len(debug_values)), samples):
                    i, j, slope, lt = debug_values[idx]
                    y_vals = V[i, j, :]
                    log_y = np.log(np.clip(y_vals, 1e-3, None))
                    fitted_line = XX @ np.array([betac[0], slope])

                    print(f"Pixel ({i},{j}): slope={slope:.6f}, lifetime={lt:.2f}")
                    ...
                    plt.plot(x_fit, fitted_line, 'r-', label=f'Fit (slope={slope:.6f})')
...
def visualize_results(base_image, lifetime_map, masks):
    """
    Visualize results to help with debugging
    """
    ...
    plt.colorbar(label='Lifetime (us)')
...
def analyze_condition_folder(folder_path, manual_circles, results_list, debug=True):
    ...
    for i, mask in enumerate(masks):
        curve, times = inspect_decay_curve(stack, mask, f"Decay curve for Sensor {i + 1}")
        # Check if curve is decreasing
        if np.mean(curve[:3]) <= np.mean(curve[-3:]):
            print(
                f"⚠️ WARNING: Sensor {i + 1} curve is not decreasing. First points: {curve[:3]}, Last points: {curve[-3:]}")

    # Calculate lifetime map with debugging
    print("\nCalculating lifetime map...")
    lifetime_map = exp_fit_func(stack, debug=True)

    # Visualize results
    if debug:
        visualize_results(base_image, lifetime_map, masks)

    # Additional debug: Check lifetime map stats
    if debug:
        print(f"\nLifetime map stats:")
        print(f"  Min: {np.min(lifetime_map)}, Max: {np.max(lifetime_map)}, Mean: {np.mean(lifetime_map):.2f}")
    ...
    for i, mask in enumerate(masks):
        ...
        masked_lifetime = exp_fit_func(stack, debug=True, mask=mask)
        ...
        if len(nonzero_tau) > 0:
            ...
            valid_percent = len(nonzero_tau) / len(tau_values) * 100
        else:
            avg_tau = 0
            std_tau = 0
            valid_percent = 0
...
def analyze_all_conditions(main_dir, debug=True):
...
def test_exp_fit():
    ...
    # Test the function
    test_lifetime_map = exp_fit_func(test_vol, debug=True, mask=test_mask)
...
if __name__ == "__main__":
    ...
    main_directory = ("/Users/timothyvernon/Documents/PhD/Projects/gMAP/SpotTest/SpotTest_05_02_25_VFA/B1_250TLBR_0TRBL")
